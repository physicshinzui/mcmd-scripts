...
c  The current c. mat. may be from subroutine modi_cmat or may not.
        call cal_denomi
c****
c  N of numzero bin positions.

        ncouele=0
        do mm=1,mvert
          iccc=0
          do ii=1,nwin
            if(cpdf(ii,mm).ne.0.0d0) iccc=iccc+1
          enddo
          ncouele=ncouele+iccc**2
        enddo
c****
...
c  Convergence check.
c  the factor fff is the relative difference of c. matrix
c  par one elements.
        ...
        fff=fff/real(ncouele)/2.0d0

        print*,'  count = ',nloop
        print*,'    change of c. mat. par an element = ',fff
